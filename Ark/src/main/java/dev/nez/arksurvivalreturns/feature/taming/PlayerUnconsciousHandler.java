...
/**
 * Server-side enforcement of the unconscious-player restrictions.
 *
 * <p>Every restriction here is a real server rule: interactions, attacks, item use, item dropping and
 * mounting are refused, and voluntary movement is clamped against the position the player fell asleep at.
...
public final class PlayerUnconsciousHandler {
    ...
    /** Death clears sedation for players; logout and login deliberately do not. */
    ...
    private static boolean deny(net.minecraft.world.entity.player.Player player, String key) {
        if (!(player instanceof ServerPlayer server) || !TorporService.restricted(server)) return false;
