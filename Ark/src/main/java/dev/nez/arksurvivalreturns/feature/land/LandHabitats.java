...
public final class LandHabitats {
    ...
    public static LandHabitatData.Habitat group(CreatureEntity mob) {
        if(mob.species().flyer()||!(mob.level() instanceof ServerLevel world)||!enabled(world))return null;
    ...
    public static LandHabitatData.Habitat think(CreatureEntity mob) {
        if(!(mob.level() instanceof ServerLevel world)||mob.species().flyer()||!enabled(world))return null;
        var h=group(mob);
        ...
        int foragers=(int)h.loaded.values().stream().filter(c -> c.isAlive() && world.isPositionEntityTicking(c.blockPosition())
                &&c.behavior()==BehaviorState.FORAGE&&forage(world,c.blockPosition())).count();
    ...
    public static void revalidate(ServerLevel world, LandHabitatData.Habitat h) {
        ...
        var result=LandWaterIndex.get(world).observe(world,h.water);
    ...
    public static boolean forage(ServerLevel world, BlockPos pos) {
        if(world.getChunkSource().getChunkNow(pos.getX()>>4,pos.getZ()>>4)==null)return false;
        var ground=world.getBlockState(pos.below());return ground.is(Blocks.GRASS_BLOCK)||ground.is(Blocks.PODZOL)||ground.is(Blocks.MYCELIUM)||ground.is(Blocks.MOSS_BLOCK)||ground.is(Blocks.PALE_MOSS_BLOCK);
    }
    private static void chooseDestination(ServerLevel world, LandHabitatData.Habitat h) {
        ...
        for(int attempt=0;attempt<4;attempt++) {
            ...
            if(p==null||Math.abs(p.getY()-h.center.getY())>6||!world.isPositionEntityTicking(p)||h.routine==BehaviorState.FORAGE&&!forage(world,p))continue;
    ...
    public static boolean atWater(ServerLevel world, LandHabitatData.Habitat h, CreatureEntity mob) {
        if(!h.valid||world.getChunkSource().getChunkNow(h.water.getX()>>4,h.water.getZ()>>4)==null)return false;
    ...
    public static void removed(CreatureEntity mob, Entity.RemovalReason reason) {
        if(mob.species().flyer()||!(mob.level() instanceof ServerLevel world))return;
        ...
        if(reason.shouldDestroy()&&h.members.remove(mob.getUUID())) {
            h.replacementAt=world.getGameTime()+Config.LAND_REPOPULATE.get();LandHabitatData.get(world).setDirty();
        }
    }
