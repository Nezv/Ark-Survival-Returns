...
import dev.nez.arksurvivalreturns.client.audio.physics.utils.RaycastUtils;
import dev.nez.arksurvivalreturns.client.audio.physics.utils.SoundRateManager;
...
import dev.nez.arksurvivalreturns.client.audio.physics.config.ReverbParams;
import dev.nez.arksurvivalreturns.client.audio.physics.debug.RaycastRenderer;
import dev.nez.arksurvivalreturns.client.audio.physics.profiling.TaskProfiler;
import dev.nez.arksurvivalreturns.client.audio.physics.profiling.TaskProfiler.TaskProfilerHandle;
import dev.nez.arksurvivalreturns.client.audio.physics.utils.LevelAccessUtils;
import dev.nez.arksurvivalreturns.client.audio.physics.world.ClientLevelProxy;
...
public class SoundPhysics {
    ...
    private static float getBlockReflectivity(BlockPos blockPos) {
        ...
        return materialReflectivity(blockState);
    }

    private static float materialReflectivity(BlockState state) {
        var type = state.getSoundType();
        if (type == net.minecraft.world.level.block.SoundType.GLASS || type == net.minecraft.world.level.block.SoundType.METAL
                || type == net.minecraft.world.level.block.SoundType.COPPER || type == net.minecraft.world.level.block.SoundType.IRON) return 0.9F;
        if (type == net.minecraft.world.level.block.SoundType.STONE || type == net.minecraft.world.level.block.SoundType.DEEPSLATE
                || type == net.minecraft.world.level.block.SoundType.TUFF) return 0.7F;
        if (type == net.minecraft.world.level.block.SoundType.WOOL || type == net.minecraft.world.level.block.SoundType.SNOW
                || type == net.minecraft.world.level.block.SoundType.MOSS) return 0.2F;
        if (type == net.minecraft.world.level.block.SoundType.WOOD || type == net.minecraft.world.level.block.SoundType.BAMBOO_WOOD) return 0.45F;
        return SoundPhysicsMod.CONFIG.defaultBlockReflectivity.get();
    }
    ...
    private static double runOcclusion(Vec3 soundPos, Vec3 playerPos) {
        ...
        for (int i = 0; i < SoundPhysicsMod.CONFIG.maxOcclusionRays.get(); i++) {
            ...
            float blockOcclusion = SoundPhysicsMod.CONFIG.defaultBlockOcclusionFactor.get();
