...
/** The two eyeballs of the visible eye set only, using a depth-tested emissive pipeline. */
final class NightEyesLayer<R extends EntityRenderState & GeoRenderState> extends GeoRenderLayer<CreatureEntity, Void, R> {
    ...
    /** Each eyeball pivots on its own centre; a slight inflation keeps the glow off the coplanar faces. */
    private static final float EYE_INFLATE = 1.04f;
    private static final Set<String> LOGGED = ConcurrentHashMap.newKeySet();
    private final String[] eyes;
    private final String[] alertEyes;
    NightEyesLayer(GeoRenderer<CreatureEntity, Void, R> renderer, Species species) {
        super(renderer);
        eyes = species.eyeBones(false);
        alertEyes = species.eyeBones(true);
    }
    ...
    @Override public void addPerBoneRender(RenderPassInfo<R> pass, BiConsumer<GeoBone, PerBoneRender<R>> consumer) {
        ...
        for (String eye : alert ? alertEyes : eyes) pass.model().getBone(eye).ifPresent(bone -> consumer.accept(bone, (p, b, collector) -> {
