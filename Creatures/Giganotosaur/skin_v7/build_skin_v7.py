...
def layer_ids(points,part,field,head,bounds,config):
    ...
    lo,hi = bounds[st.PART_ID[part],2:4]
...
def save_previews(out,geo,cubes,skeleton,style,report,nose):
    ...
    label = out.parent.parent.name if out.parent.name == 'skin_v7' else out.parent.name
...
def build(creature,config,dry_run=False,previews=True,output_name='skin_v7'):
    ...
    report['variant'] = output_name.split('/')[-1] if output_name != 'skin_v7' else 'base'
...
def main():
    ...
    parser.add_argument('--variants',nargs='*',metavar='NAME',help='Also build named palette variants into skin_v7/<NAME>.')
