...
- **Field medicine, second phase (P04).** Three narcoberries and a plant fiber now craft into a **concentrated sedative** (100 potency by default, configurable), usable through every route the berry has. Four base tranquilizer arrows plus one concentrate craft into four **improved tranquilizer arrows** that carry the stronger dose. Both recipes work at the crafting table; no medicine station is required.
- **Kitchen, third phase (P04).** The **cooking pot** holds four ingredients and cooks on the same batch cadence as the other stations. Two fixed meals are available: **hearty stew** (two dried rations, one meat, one carrot; regeneration) and **trail mix** (two dried rations, two Ark berries; a short speed effect). All four ingredient slots must be filled, each slot loses exactly one item per meal, and the pot's own screen reuses a vanilla panel; new `[kitchen]` settings control the station and the batch count.
- **Forge stations, fourth phase (P04).** The **charcoal kiln** reduces logs, planks and saplings to charcoal with no fuel, and the **primitive forge** processes raw iron, copper and gold, sand, cobblestone, deepslate, clay and dried rations without fuel over a configurable number of batches. Both are one-input, one-output stations: an empty hand takes the finished item first, then the input back, and contents drop when the block is removed. The vanilla furnace remains fully usable; `[forge]` settings control the stations and their batch counts.
- **Storage crate, fifth phase (P04).** A fixed **27-slot storage crate** crafts from a chest, planks and fiber and opens a vanilla-layout screen. It holds items and nothing else - no ticking, no automation - and its contents survive save/load and drop when it is broken.
- **Validation.** Data generation, the build with 52 JUnit tests and all 55 headless GameTests pass, `tools/verify_assets.py` validates 73 item definitions, and FTB Quests loads four chapters and twenty-three quests with every item task and recipe resolving. The full in-client checklist is in [Verify.md](Verify.md).

## Weight and Working Tames — 2026-09-20

- **Carried mass, first phase (P03).** Players now haul a measured load: item mass comes from data-pack tags with a stack-size fallback for anything unknown, containers are inspected one level deep with a capped content sum and never recursed, and equipment counts. A top-left HUD gauge shows the load; up to 75% is free, 75–100% warns without any penalty, and only above 100% does sprint lock and movement slow toward a configurable floor. Nothing blocks item movement, so deliberate overloading still works for rearranging or dropping cargo. The `[mass]` section adds a relaxed preset, an off switch and the Fast Load ceiling reserved for the creature-cargo phase.
- **Cargo rigs and working haulers, second phase (P03).** Two harnesses — a pack harness from leather and fiber, and a reinforced harness built from it — fit the new rig slot in the tame screen. Each species declares a hauling role: the required harness unlocks its cargo capacity, a lighter rig leaves only the bare allowance, and reinforcements are required by the heavy haulers and giants. While mounted the rider's carried load counts against the mount, which slows under overload; nothing blocks slot moves, so deliberate overloading stays possible. **Load** and **Unload** buttons bulk-transfer between the hold and nearby storage, reading loaded chunks only and stopping Fast Load before the configurable automation ceiling. The tame screen shows `Cargo X / Y`, and the HUD switches to the mount bar while riding.
- **Overload behavior, third phase (P03).** An overloaded flyer refuses to take off, and one already airborne descends under powered control rather than falling. An overloaded swimmer cannot dive, becomes slightly buoyant and surfaces outright when the rider's air runs low, so heavy cargo never becomes an unavoidable drowning. Riders get rate-limited warnings, and autonomous tamed haulers fly slower under load.
- **Work orders and harvesting, fourth phase (P03).** The companion whistle now cycles a fourth **WORK** order for worker species: a Triceratops grazes grass in place, cuts tufts and resets ripe berry bushes for plant fiber and berries, while an Ankylosaurus mines natural stone and ores with a configurable bonus. Jobs anchor where the order was given, sample only loaded chunks inside a bounded radius, never harvest player-placed blocks (tracked per dimension), and pause when no authorized survivor stays within range, so a worker is never unattended automation. `/arktribe perm <player> work on|off` and `[tribe] defaultWork` extend the permission layer, and the job stops at the cargo automation ceiling. A third journal chapter, **Working Tames**, adds five objectives for rigs, work orders and their yields.
- **Validation.** Six JUnit threshold and permission tests and the `mass_load`, `cargo_load`, `cargo_transfer`, `overload_flight`, `overload_swim` and `work_harvest` GameTests (chunk-count checked); all 50 headless GameTests, 52 JUnit tests, the full build, `tools/verify_assets.py` and FTB Quests loading 3 chapters / 17 quests pass. The full in-client checklist for P03 is in [Verify.md](Verify.md).
